...
public class PreviewMediaActivity extends BasicActivity<ActivityPreviewBinding> {
    ...
    private void initView() {
        List<PreviewMediaVM.MediaData> list = new ArrayList<>(vm.mediaDataList);
        int initIndex = vm.currentIndex;
        view.pager.setAdapter(new MediaPagerAdapter(list));
        view.pager.setCurrentItem(initIndex);
        PreviewMediaVM.MediaData mediaData = list.get(initIndex);
        ...
        view.pager.addOnPageChangeListener(new ViewPager.OnPageChangeListener() {
            ...
            public void onPageSelected(int position) {
                ...
                view.download.setVisibility((TextUtils.isEmpty(list.get(position).mediaUrl) || !list.get(position).mediaUrl.startsWith("http")) ? View.GONE : View.VISIBLE);
        ...
        if (list.size() == 1) {
            ...
            view.download.setVisibility(list.get(0).mediaUrl.startsWith("http")
                ? View.VISIBLE : View.GONE);
        } else addGuideView(list, initIndex);


        view.download.setOnClickListener(v -> {
            hasWrite.safeGo(() -> {
                String originUrl = list.get(view.pager.getCurrentItem()).mediaUrl;
                ...
                Common.downloadFile(originUrl, null, targetUri).subscribe(new NetObserver<Integer>(this) {
                    @Override
                    public void onSuccess(Integer progress) {
                        Common.UIHandler.post(() -> {
                            if (progress >= 0 && progress <= 100) {
                                downloadDialog.setProgress(progress);
                                downloadDialog.setInfo(progress + "%");
                                if (progress == 100) {
                                ...
                                Toast.makeText(BaseApp.inst(), tips, Toast.LENGTH_SHORT).show();
//                                contentResolver.delete(targetUri, null)
                                releaseDownloadManagerRes();
                            }
                        });
                    }
