...
public class IMUtil {
    ...
    public static RequestBuilder<?> loadPicture(PictureElem elem) {
        ...
        if (TextUtils.isEmpty(url) && null != elem.getSnapshotPicture() && !"image/gif".equals(elem.getSourcePicture().getType()))
            url = elem.getSnapshotPicture().getUrl();
        if (TextUtils.isEmpty(url)) {
            url = elem.getSourcePicture().getUrl();
        }
        return Glide.with(BaseApp.inst()).load(url).placeholder(new PlaceHolderDrawable(BaseApp.inst())).error(R.mipmap.ic_chat_photo);
    }
    ...
    public static RequestBuilder<?> loadVideoSnapshot(VideoElem elem) {
        ...
        return Glide.with(BaseApp.inst()).load(path).placeholder(new PlaceHolderDrawable(BaseApp.inst())).error(R.mipmap.ic_chat_photo);
    }
    ...
    public static Message buildExpandInfo(Message msg) {
        ...
        try {
            ...
            if (msg.getContentType() == MessageType.AT_TEXT) {
                ...
                handleAtMsg(msgExpand, msg.getGroupID());
...
//            e.printStackTrace();
    ...
    private static void handleAtMsg(MsgExpand msgExpand, String gid) {
        ...
        assert spannableString != null;
        handleHyperLink(spannableString);
