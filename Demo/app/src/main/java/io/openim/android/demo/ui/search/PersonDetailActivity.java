...
public class PersonDetailActivity extends BaseActivity<SearchVM, ActivityPersonDetailBinding> implements Observer {
    ...
    private final Consumer<FriendApplicationInfo> friendStateConsumer = friendApplicationInfo ->  {
        if (TextUtils.equals(friendApplicationInfo.getToUserID(), vm.searchContent.val()) && friendApplicationInfo.getHandleResult() != 0) {
            vm.isFriend.setValue(friendApplicationInfo.getHandleResult() == 1);
        }
    };
    ...
    private boolean isHideAdd() {
        ...
        return applyMemberFriend || isFriend || oneself()  || isInBlackList;
    }
